...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .download-select:focus {{ border-color: var(--cyan); }}
    .btn-download {{
      background: linear-gradient(135deg, #00adb5, #007c82);
      border: none;
      color: #fff;
      padding: 6px 14px;
      border-radius: 6px;
    ...
    .btn-download-secondary:hover {{ background: #2f2f3a; color: #fff; }}
    .download-status {{
      font-size: 0.78rem;
      color: var(--green);
      font-weight: 600;
    ...
    @media (max-width: 600px) {{
      ...
      .download-select {{ max-width: 100%; width: 100%; }}
    }}
  </style>
</head>
<body>
  ...
  <div id="player-section">
    ...
    <!-- Download Options Bar with Counting Pure Watch-Time Dropdown -->
    <div class="player-download-bar">
      <div class="download-label">
        <i class="fa-solid fa-cloud-arrow-down" style="color: var(--cyan);"></i>
        <span>Download Video & CC:</span>
      </div>
      <div class="download-controls">
        <select id="download-batch-select" class="download-select">
          <option value="1">1 ep (21 min)</option>
        </select>
        <button id="download-btn" class="btn-download" onclick="startBatchDownload()">
          <i class="fa-solid fa-download"></i> <span id="download-btn-text">Download MP4 + Subtitles</span>
        </button>
        <button class="btn-download-secondary" onclick="openDownloadLinksModal()" title="Export Direct Links & Scripts">
          <i class="fa-solid fa-terminal"></i> Links / Scripts
        </button>
      </div>
  ...
  <!-- Download Links / Script Modal -->
  <div id="download-modal" class="modal-backdrop" onclick="closeDownloadLinksModal(event)">
    <div class="modal-card" onclick="event.stopPropagation()">
      <div class="modal-header">
        <h3><i class="fa-solid fa-list-check" style="color:var(--cyan);"></i> Batch Download URLs & Script</h3>
        <button class="modal-close" onclick="closeDownloadLinksModal()"><i class="fa-solid fa-xmark"></i></button>
      </div>
      <p style="font-size: 0.8rem; color: var(--muted);">Direct MP4 and Subtitle URLs for the selected batch. Copy and paste into terminal (cURL/aria2c) or your download manager.</p>
      <div class="modal-body">
        <textarea id="download-modal-text" readonly></textarea>
      </div>
      <div class="modal-footer">
        <button class="btn-download" onclick="copyModalScript()"><i class="fa-solid fa-copy"></i> Copy to Clipboard</button>
        <button class="btn-download-secondary" onclick="closeDownloadLinksModal()">Close</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    const downloadBatchSelect = document.getElementById('download-batch-select');
    const downloadStatus = document.getElementById('download-status');
    const downloadModal = document.getElementById('download-modal');
    const downloadModalText = document.getElementById('download-modal-text');
    ...
    // Time Between Endings Calculation (content duration up to ed_start)
    function getTimeBetweenEndings(ep) {{
      const ts = ep.timestamps || {{}};
      const opEnd = (ts.op_end != null) ? ts.op_end : 0;
      let edStart;

      if (ts.ed_start != null) {{
        edStart = ts.ed_start;
      }} else if (ep.id === 'S04E29') {{
        edStart = 59 * 60;
      }} else if (ep.id === 'S04E30') {{
        edStart = 83 * 60;
      }} else if (ep.type === 'ova') {{
        edStart = 25 * 60;
      ...
      return Math.max(60, Math.round(edStart - opEnd));
    }}

    function formatNetDuration(sec) {{
    ...
    function updateDownloadDropdown() {{
      if (!downloadBatchSelect || currentEpisodeIndex === -1) return;
      downloadBatchSelect.innerHTML = '';

      const maxAvailable = Math.min(6, allEpisodes.length - currentEpisodeIndex);
      const currentEp = allEpisodes[currentEpisodeIndex];
      let cumulativeSec = 0;

      for (let count = 1; count <= maxAvailable; count++) {{
        const ep = allEpisodes[currentEpisodeIndex + count - 1];
        cumulativeSec += getTimeBetweenEndings(ep);
        const timeStr = formatNetDuration(cumulativeSec);

        const opt = document.createElement('option');
        opt.value = count;
        if (count === 1) {{
          opt.textContent = `1 ep (${{timeStr}} • ${{currentEp.id}})`;
        }} else {{
          opt.textContent = `${{count}} eps (${{timeStr}} • ${{currentEp.id}} - ${{ep.id}})`;
        }}
        downloadBatchSelect.appendChild(opt);
      }}
    }}
      ...
      fetch('episodes.json?v=' + Date.now())
        ...
        .then(data => {{
          if (Array.isArray(data) && data.length > 0) {{
            ...
            renderEpisodes();
            checkAndShowResumePrompt();
            if (currentEpisodeIndex !== -1) updateDownloadDropdown();
          }}
        }})
        .catch(err => {{
    ...
    function playEpisode(epId, startTime = 0) {{
      ...
      // Populate Download Dropdown with Counting Net Durations
      updateDownloadDropdown();
    ...
    async function startBatchDownload() {{
      ...
      const count = parseInt(downloadBatchSelect ? downloadBatchSelect.value : '1') || 1;
      const items = allEpisodes.slice(currentEpisodeIndex, currentEpisodeIndex + count);

      if (downloadStatus) {{
        downloadStatus.style.display = 'block';
        downloadStatus.textContent = `Starting download of ${{items.length}} episode(s)...`;
      }}

      for (let i = 0; i < items.length; i++) {{
        const ep = items[i];
        if (downloadStatus) {{
          downloadStatus.textContent = `Triggering download (${{i + 1}}/${{items.length}}): ${{ep.id}} - ${{ep.title || ep.filename}}...`;
        }}

        // 1. Download Video
        downloadFileUrl(ep.stream_url, `${{ep.id}}_${{ep.filename || 'video.mp4'}}`);

        // 2. Download Subtitle
        const subFile = ep.subtitle_path || ep.vtt_path;
        if (subFile) {{
          downloadFileUrl(subFile, `${{ep.id}}_English.srt`);
        }}
      ...
      if (downloadStatus) {{
        downloadStatus.textContent = `✓ Triggered downloads for ${{items.length}} episode(s)! Check your browser downloads bar.`;
        setTimeout(() => {{ if (downloadStatus) downloadStatus.style.display = 'none'; }}, 8000);
      }}
    }}
    ...
    // Export Links / Batch Script Modal
    function openDownloadLinksModal() {{
      if (currentEpisodeIndex === -1) return;
      const count = parseInt(downloadBatchSelect ? downloadBatchSelect.value : '1') || 1;
      const items = allEpisodes.slice(currentEpisodeIndex, currentEpisodeIndex + count);

      let text = "# Attack on Titan - Direct Batch Download URLs\\n";
      text += `# Batch: ${{items[0].id}} to ${{items[items.length - 1].id}} (${{items.length}} episodes)\\n\\n`;
      text += "# 1. Video URLs (MP4):\\n";
      items.forEach(ep => {{
        text += `${{ep.stream_url}}\\n`;
      }});
      ...
      text += "\\n# 2. Subtitle URLs (SRT):\\n";
      items.forEach(ep => {{
        const sub = ep.subtitle_path || ep.vtt_path;
        if (sub) text += `https://asaploso.github.io/attack-on-titan-streamer/${{sub}}\\n`;
      }});

      text += "\\n# 3. Ready-to-Run cURL Script (Mac/Linux/Windows):\\n";
      items.forEach(ep => {{
        text += `curl -L -O "${{ep.stream_url}}"\\n`;
      }});

      downloadModalText.value = text;
      downloadModal.style.display = 'flex';
    }}

    function closeDownloadLinksModal() {{
      downloadModal.style.display = 'none';
    }}

    function copyModalScript() {{
      downloadModalText.select();
      navigator.clipboard.writeText(downloadModalText.value).then(() => {{
        alert("Copied download URLs & cURL script to clipboard!");
      }});
    }}
